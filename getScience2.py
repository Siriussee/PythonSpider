...
outlets = re.findall(pattern_outlet,html_text)[0].split('<b>')[1].split('</b>')[0]
print outlets

# Blogged by <b>24</b>
pattern_blogged = re.compile(r'Blogged by <b>\d+</b>')
blogged = re.findall(pattern_blogged,html_text)[0].split('<b>')[1].split('</b>')[0]
print blogged

# Tweeted by <b>260</b>
pattern_tweet = re.compile(r'Tweeted by <b>\d+</b>')
tweeted = re.findall(pattern_tweet,html_text)[0].split('<b>')[1].split('</b>')[0]
print tweeted

# On <b>13</b> Facebook pages
pattern_facebook = re.compile(r'On <b>\d+</b> Facebook pages')
facebooked = re.findall(pattern_facebook,html_text)[0].split('<b>')[1].split('</b>')[0]
print facebooked

# Referenced in <b>7</b> Wikipedia pages
pattern_wiki = re.compile(r'Referenced in <b>\d+</b> Wikipedia pages')
wikied = re.findall(pattern_wiki,html_text)[0].split('<b>')[1].split('</b>')[0]
print wikied

# Mentioned in <b>8</b> Google+ posts
pattern_gplus =re.compile(r'Mentioned in <b>\d+</b>')
gplus = re.findall(pattern_gplus,html_text)[0].split('<b>')[1].split('</b>')[0]
print gplus

# Reddited by <b>1</b>
pattern_reddit = re.compile(r'Reddited by <b>\d+</b>')
reddited = re.findall(pattern_reddit,html_text)[0].split('<b>')[1].split('</b>')[0]
print reddited

# On <b>2</b> videos
pattern_vedio = re.compile(r'On <b>\d+</b> videos')
videoed = re.findall(pattern_vedio,html_text)[0].split('<b>')[1].split('</b>')[0]
print videoed

# <b>44</b> readers on Mendeley
pattern_mendeley = re.compile(r'<b>\d+</b> readers on Mendeley')
mendeleyed = re.findall(pattern_mendeley,html_text)[0].split('<b>')[1].split('</b>')[0]
print mendeleyed
...
data_list = [DOI,catagory,pubilcation_date,title,altmetric_score,outlets,blogged,tweeted,facebooked,gplus,wikied,videoed,reddited,mendeleyed]
